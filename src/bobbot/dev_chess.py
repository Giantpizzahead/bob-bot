...
async def get_challenge_link(page: Page) -> str:
    ...
    print(f"Challenge link: {challenge_link}")
...
async def wait_for_accepted_match(page: Page) -> None:
    ...
    print("Waiting for match to be accepted...")
    try:
...
async def wait_for_move(page: Page) -> None:
    ...
    try:
        if await page.locator(".clock-bottom").is_visible():
            ...
            print("Waiting for move list update...")
            await page.wait_for_function(
...
async def screenshot_board(page: Page) -> Image:
    """Take a screenshot of the whole page (including the board)."""
...
async def main():
    ...
    async with async_playwright() as playwright:
        ...
        print(f"Final result: {match_result}")
        img: Image = await screenshot_board(page)
        img.save("local/pw/board.png")
        img.show()
